...
export default function DashboardPage() {
  ...
  const dailyChartData = dailyFeedbackData
    ...
    : (import.meta.env.DEV ? [
        { time: 3, value: 45, emotion: "neutral" },
        { time: 6, value: 60, emotion: "happiness" },
        { time: 9, value: 75, emotion: "happiness" },
        { time: 12, value: 80, emotion: "happiness" },
        { time: 15, value: 70, emotion: "happiness" },
        { time: 18, value: 55, emotion: "neutral" },
        { time: 21, value: 40, emotion: "sadness" },
        { time: 24, value: 35, emotion: "sadness" }
      ] : []); // 개발 환경에서만 예시 데이터 사용

  // API 응답을 Week 차트 데이터 형식으로 변환
  // WeeklyEmotionDto: { weekRange: { start, end }, emotions: [{ day, emotion }] }
  // EmotionDayDto: { day: integer, emotion: integer }
  const convertWeekDataToChartFormat = (apiData) => {
    ...
    return apiData.emotions.map((emotion) => ({
      day: emotion.day,
      value: emotion.emotion !== undefined ? emotion.emotion : emotion.score // emotion 필드 우선, 없으면 score 사용
    }));
  };
  ...
  const convertMonthDataToCalendarFormat = (apiData) => {
    ...
    // API 응답 형식: { day: number, emotion: number }
    const calendarMap = {};
    apiData.days.forEach((item) => {
      if (item.day !== undefined && item.emotion !== undefined) {
  ...
  const monthCalendarDataMap = monthFeedbackData
    ...
    : (import.meta.env.DEV ? {
        // 예시 데이터: 다양한 날짜와 감정 값으로 구성 (숫자 값 사용)
        // 감정 분석 모델 범위:
        // 0-16: 매우 부정적 (depression), 17-33: 부정적 (anger/sadness)
        // 34-50: 중립 (neutral/anxiety), 51-66: 약간 긍정적 (neutral)
        // 67-83: 긍정적 (happiness), 84-100: 매우 긍정적 (happiness)
        1: 75, // 긍정적 (happiness)
        2: 82, // 매우 긍정적 (happiness)
        3: 45, // 중립 (neutral)
        4: 68, // 긍정적 (happiness)
        5: 38, // 중립 (anxiety)
        6: 90, // 매우 긍정적 (happiness)
        7: 47, // 중립 (neutral)
        8: 72, // 긍정적 (happiness)
        9: 28, // 부정적 (sadness)
        10: 85, // 매우 긍정적 (happiness)
        11: 65, // 약간 긍정적 (neutral)
        12: 44, // 중립 (neutral)
        13: 78, // 긍정적 (happiness)
        14: 36, // 중립 (anxiety)
        15: 88, // 매우 긍정적 (happiness)
        16: 12, // 매우 부정적 (depression)
        17: 70, // 긍정적 (happiness)
        18: 46, // 중립 (neutral)
        19: 80, // 매우 긍정적 (happiness)
        20: 30, // 부정적 (sadness)
        21: 92, // 매우 긍정적 (happiness)
        22: 58, // 약간 긍정적 (neutral)
        23: 40, // 중립 (anxiety)
        24: 76, // 긍정적 (happiness)
        25: 48, // 중립 (neutral)
        26: 84, // 매우 긍정적 (happiness)
        27: 66, // 약간 긍정적 (neutral)
        28: 8, // 매우 부정적 (depression)
        29: 95, // 매우 긍정적 (happiness)
        30: 42, // 중립 (neutral)
        31: 73 // 긍정적 (happiness)
      } : {}); // 개발 환경에서만 예시 데이터 사용
  ...
  // 감정 분석 모델 범위:
  // ① 0-16: 매우 부정적 (절망, 무기력, 불안, 공허) -> depression
  // ② 17-33: 부정적 (짜증, 실망, 걱정, 예민함, 답답함) -> anger/sadness
  // ③ 34-50: 중립 (담담함, 무난함, 심심함, 어수선함) -> neutral/anxiety
  // ④ 51-66: 약간 긍정적 (차분함, 안정감, 편안함, 여유로움) -> neutral/happiness
  // ⑤ 67-83: 긍정적 (즐거움, 만족, 활기참, 설렘, 뿌듯함) -> happiness
  // ⑥ 84-100: 매우 긍정적 (행복, 감동, 신남, 황홀함, 기쁨) -> happiness
  const getEmotionByValue = (value) => {
    ...
    if (value >= 0 && value <= 16) {
      ...
      // 약간 긍정적: 차분함, 안정감, 편안함, 여유로움
      return { character: "neutral", emotion: "편안함", message: "편안한 하루를 계속하도록, Localy가 도와드릴게요." };
    } else if (value >= 67 && value <= 83) {
      // 긍정적: 즐거움, 만족, 활기참, 설렘, 뿌듯함
      return { character: "happiness", emotion: "즐거움", message: "즐거운 하루를 계속하도록, Localy가 도와드릴게요." };
    } else if (value >= 84 && value <= 100) {
      // 매우 긍정적: 행복, 감동, 신남, 황홀함, 기쁨
      return { character: "happiness", emotion: "행복함", message: "행복한 하루를 계속하도록, Localy가 도와드릴게요." };
    }
    return { character: "neutral", emotion: "중립", message: "균형잡힌 하루를 보내도록, Localy가 도와드릴게요." };
  };

  // API의 monthlyStats를 기반으로 감정별 횟수 계산
  // monthlyStats 형식: { e1: number, e2: number, e3: number, e4: number, e5: number, e6: number }
  // e1: happiness, e2: sadness, e3: anger, e4: depression, e5: neutral, e6: anxiety
  const calculateEmotionCounts = () => {
    // API에서 monthlyStats를 제공하는 경우 사용
    if (monthFeedbackData && monthFeedbackData.monthlyStats) {
      const stats = monthFeedbackData.monthlyStats;
      return [
        { emotion: "happiness", count: stats.e1 || 0 },
        { emotion: "sadness", count: stats.e2 || 0 },
        { emotion: "anger", count: stats.e3 || 0 },
        { emotion: "depression", count: stats.e4 || 0 },
        { emotion: "neutral", count: stats.e5 || 0 },
        { emotion: "anxiety", count: stats.e6 || 0 },
      ];
    }

    // monthlyStats가 없는 경우 캘린더 데이터를 기반으로 계산 (예시 데이터용)
    const counts = {
      happiness: 0,
      sadness: 0,
      anger: 0,
      ...
      <S.Container>
          ...
          <S.ChartSection>
            ...
            <S.ChartArea>
              ...
              {selectedPeriod === "Daily" && (
                <>
                  <S.GridLine $top={60} $left={28} />
                  <S.GridLine $top={96} $left={28} />
                  <S.GridLine $top={132} $left={28} />
                  <S.GridLine $top={168} $left={28} />
                  <S.GridLine $top={204} $left={28} />
                  <S.GridLine $top={240} $left={28} />

                  {/* X축 라벨 */}
                  <S.XAxisLabel $left={28}>3</S.XAxisLabel>
                  <S.XAxisLabel $left={66.86}>6</S.XAxisLabel>
                  <S.XAxisLabel $left={105.71}>9</S.XAxisLabel>
                  <S.XAxisLabel $left={144.57}>12</S.XAxisLabel>
                  <S.XAxisLabel $left={183.43}>15</S.XAxisLabel>
                  <S.XAxisLabel $left={222.29}>18</S.XAxisLabel>
                  <S.XAxisLabel $left={261.14}>21</S.XAxisLabel>
                  <S.XAxisLabel $left={300}>24</S.XAxisLabel>
                </>
              )}
              ...
              {selectedPeriod === "Daily" && (
                <>
                  <S.EmotionCharacter $left={11} $top={60}>
                    {/* 행복 */}
                  ...
                  <S.EmotionCharacter $left={11} $top={96}>
                    {/* 중립 */}
                  ...
                  <S.EmotionCharacter $left={11} $top={132}>
                    {/* 불안 */}
                  ...
                  <S.EmotionCharacter $left={11} $top={168}>
                    {/* 우울 */}
                  ...
                  <S.EmotionCharacter $left={11} $top={204}>
                    {/* 슬픔 */}
                  ...
                  <S.EmotionCharacter $left={11} $top={240}>
                    {/* 분노 */}
        ...
        {selectedPeriod === "Month" && (
          <S.MonthListSection>
            ...
            {monthEmotionCounts.filter(item => item && item.emotion).length > 0 ? (
              <S.MonthEmotionList>
